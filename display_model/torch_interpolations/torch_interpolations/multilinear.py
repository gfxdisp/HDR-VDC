...
class RegularGridInterpolator:
    ...
    def to_device(self, device):
        for i in range(self.n):
            self.points[i] = self.points[i].to(device)
    ...
    def __call__(self, points_to_interp):
        ...
        for indexer in product([0, 1], repeat=self.n):
            ...
            numerator += self.values[as_s] * torch.prod(torch.stack(bs_s), dim=0)[..., None]
        denominator = torch.prod(torch.stack(overalls), dim=0)[..., None]
